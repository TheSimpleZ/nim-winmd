...
# or, for WinRT metadata (Windows.winmd), the modules of winmd/winrtgen.nim;
# the options and the map are for Win32 metadata, and refused for WinRT.

import std/[cmdline, dirs, files, paths, syncio, times, strutils, tables]
import ./winmd/[rdlmap, reader, model, generator, winrtgen]
...
let isWinrt = isWinrtMetadata(wa)
if isWinrt and (emitHeaders or lowerFirst or args.len >= 3):
  echo "--headers, --lowercase and the map are for Win32 metadata; ",
    args[0], " is WinRT metadata"
  quit(1)
...
if isWinrt:
  discard # the WinRT modules are laid out by namespace
elif dirExists(Path(mapFile)):
...
let mods =
  if isWinrt:
    generateWinrtModules(m)
  else:
    generateModules(m, typeHdr, emitHeaders, lowerFirst)
var total = 0
for gm in mods:
